...
choix<- list()
choix[["visibilite"]]<-c("0-10","11-50","51-100",">100","Nuit")
choix[["habitat"]]<-dbGetQuery(con,"select distinct (t_capture_cpt.cpt_lache_habitat_lache) from cmpt.t_capture_cpt order by cpt_lache_habitat_lache ASC")
choix[["habitat_perte"]]<-dbGetQuery(con,"select distinct (t_capture_cpt.cpt_lache_habitat_pertevue) from cmpt.t_capture_cpt order by cpt_lache_habitat_pertevue ASC")
choix[["tiques"]]<-c(0:30,'>30')
choix[["diarrhee"]]<-list(TRUE,FALSE)
choix[["lactation"]]<-c("oui", "non", "indeterminé")
choix[["Nbre_pers_experimentes"]]<-c(0:5)
choix[["sexe"]]<-c("M","F")
...
choix[["idTagOrG2"]]<-dbGetQuery(con, "select distinct cap_tag_gauche from public.t_capture_cap")
choix[["idTagOrD2"]]<-dbGetQuery(con, "select distinct cap_tag_droit from public.t_capture_cap")
choix[["idRFID2"]]<-dbGetQuery(con,"select rfi_tag_code from public.t_rfid_rfi, public.t_capture_cap, public.t_animal_ani where cap_id = rfi_cap_id and cap_ani_id = ani_id")
choix[["idSite2"]]<-dbGetQuery(con, "select sit_nom_court from public.tr_site_capture_sit where (sit_id in (select cap_sit_id from public.t_capture_cap, t_animal_ani))")
choix[["idRFID_new"]]<-dbGetQuery(con,"select rfi_tag_code from public.t_rfid_rfi where rfi_cap_id is null")
choix[["age"]]<-c("0.5" = "<1", "1.5" = "1", "2.5" = "2", "3.5" = '3', "4.5-5.5"= "4-5", ">6.5" = ">=6")
